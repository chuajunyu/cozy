...
export default function SunlightControls({ scene, catalog, onChange }: { scene: Scene; catalog: Product[]; onChange: (scene: Scene) => void }) {
  ...
  return <section className="sunlight-panel" aria-label="Windows and sunlight">
    ...
    <details><summary>Edit windows <span>{windows.length} installed</span></summary>
      ...
      <label className="window-enabled"><input type="checkbox" checked={!!window} onChange={e => onChange({ ...scene, windows: e.target.checked ? [...windows, { wall, offset: 0.5, width: 1.8, height: 1.4, sill: 0.9 }] : windows.filter(w => w.wall !== wall) })} /> Window on {wall} wall</label>
      {window && <div className="window-inputs">
        ...
        <label>Height · {window.height.toFixed(1)} m<input aria-label="Window height" type="range" min="0.5" max={Number((2.5 - window.sill).toFixed(1))} step="0.1" value={window.height} onChange={e => update({ height: Number(e.target.value) })} /></label>
        <label>Sill above floor · {window.sill.toFixed(1)} m<input aria-label="Window sill height" type="range" min="0.2" max={Number((2.5 - window.height).toFixed(1))} step="0.1" value={window.sill} onChange={e => update({ sill: Number(e.target.value) })} /></label>
      </div>}
    </details>
    <small>Illustrative solar time: sunrise 06:00 east, sunset 18:00 west. Clear sky, southern midday arc; location, date and surrounding buildings are not modeled. Windows and open doors admit daylight from outside. Diffuse sky light and surface reflections illuminate the interior. Indoor exposure adapts automatically.</small>
  </section>
}
