...
export function doorGeometry(item: Item, product: Product, scene: Pick<Scene, 'width' | 'depth'>) {
...
export function normalizeDoor(item: Item, product: Product, scene: Pick<Scene, 'width' | 'depth'>): Item {
  ...
  return {
    ...item,
    x: wall === 'west' ? 0 : wall === 'east' ? scene.width : center,
    z: wall === 'north' ? 0 : wall === 'south' ? scene.depth : scene.depth - center,
...
export function doorClearance(item: Item, product: Product, scene: Pick<Scene, 'width' | 'depth'>) {
  const { wall, start, end, width, height } = doorGeometry(item, product, scene)
  if (wall === 'north' || wall === 'south') {
    return { minX: start, maxX: end, minZ: wall === 'north' ? 0 : scene.depth - width, maxZ: wall === 'north' ? width : scene.depth, height }
  }
  return { minX: wall === 'west' ? 0 : scene.width - width, maxX: wall === 'west' ? width : scene.width, minZ: scene.depth - end, maxZ: scene.depth - start, height }
}

export function validDoorAnchor(item: Item, product: Product, scene: Pick<Scene, 'width' | 'depth'>) {
  ...
  const opening = doorGeometry(item, product, scene)
  if (opening.width + .4 > opening.length || opening.height > 2.5 || scene.width < opening.width || scene.depth < opening.width) return false
