...
export function validItemGeometry(item: Item, product: Product, scene: Scene) {
  ...
  if (
    ...
    y < 0 ||
    y + product.dimensions[1] > ROOM_HEIGHT + EPSILON
  )
    return false
  ...
  return (
    box.x - halfWidth >= -EPSILON &&
    box.z - halfDepth >= -EPSILON &&
    box.x + halfWidth <= scene.width + EPSILON &&
    box.z + halfDepth <= scene.depth + EPSILON
  )
}
...
export function settleItem(item: Item, scene: Scene, catalog: Product[]): Item | null {
  ...
  if (!isRug(product)) {
    for (const other of scene.items) {
      ...
      if (!obstacle || obstacle.door || isRug(obstacle)) continue
...
export function settleScene(
  ...
  function resolve(id: string): Item | undefined {
    ...
    const normalizingUnsupportedLegacyItem =
      next === previous && !oldSupport && !old?.supportId
    if (
      old?.locked &&
      original.locked &&
