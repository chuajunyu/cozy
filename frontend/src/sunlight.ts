...
export function validWindows(value: unknown): value is RoomWindow[] {
  return Array.isArray(value) && value.length <= 4 && new Set(value.map(w => w?.wall)).size === value.length && value.every(w => w && walls.includes(w.wall) && [w.offset, w.width, w.height, w.sill].every(Number.isFinite) && w.offset >= 0 && w.offset <= 1 && w.width >= 0.5 && w.width <= 4 && w.height >= 0.5 && w.sill >= 0.2 && w.height + w.sill <= 2.5)
}
