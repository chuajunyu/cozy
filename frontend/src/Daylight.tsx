...
export default function Daylight({ room, catalog }: { room: Scene; catalog: Product[] }) {
  ...
  const uniforms = useMemo(() => ({
    ...
    daylightExtent: { value: new Vector3(room.width, 2.7, room.depth) },
  }), [textures])

  useEffect(() => {
    const instance = new Worker(new URL('./daylight.worker.ts', import.meta.url), { type: 'module' })
    ...
    instance.onerror = () => { setFailed(true); setPending(false) }
    return () => {
      instance.terminate()
      worker.current = null
      busy.current = false
  ...
  useFrame(() => {
    ...
    uniforms.daylightExtent.value.set(room.width, 2.7, room.depth)
    ...
    const input: TransportInput = {
      width: room.width, depth: room.depth, windows: room.windows ?? defaultWindows,
  ...
  return pending || failed ? <Html position={[0, 2.85, 0]} center style={{ pointerEvents: 'none' }}><span className="daylight-progress">{failed ? 'Daylight calculation unavailable' : 'Updating daylight…'}</span></Html> : null
}
